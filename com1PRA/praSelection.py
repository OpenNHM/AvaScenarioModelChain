...
def runPraSelection(cfg, workFlowDir):
    ...
    # ----------------------------------------------------------------------
    # Optional masking by COMMISSIONREGION GeoJSON
    # ----------------------------------------------------------------------
    if maskCommRegion:
        commRegionName = cfg['MAIN'].get('COMMISSIONREGION', '').strip()
        if not commRegionName:
            raise ValueError("maskCommRegion=True but [MAIN].COMMISSIONREGION is missing in the INI file.")
        ...
        regionCrs = getattr(commGdf, "crs", None)
        if regionCrs is not None and regionCrs != demCrs:
            commGdf = commGdf.to_crs(demCrs)

        # Rasterize region polygon to binary mask aligned to DEM
        ...
        # Mask the PRA values outside the region
